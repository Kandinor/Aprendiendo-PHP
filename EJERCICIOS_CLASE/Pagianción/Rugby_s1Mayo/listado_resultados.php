...
define('NUM_ELEM_POR_PAG', 5); // Número de partidos por página
...
$orderby = isset($_GET['orderby']) && is_numeric($_GET['orderby']) && $_GET['orderby'] <= NUM_COLUMNS? $orderby_columns[$_GET['orderby']] : 'PAIS';
...
function generateQueryString($orderapintar, $orderby, $order, $orderby_columns) {
    $neworder = ($order == "ASC" && $orderapintar == $orderby)? "DESC" : "ASC";
    return "?orderby=". array_search($orderapintar, $orderby_columns). "&order=$neworder&page=1";
}
...
<body>
    ...
    <table>
        <thead>
            <tr>
                <th><a href="<?= generateQueryString('PAIS', $orderby, $order, $orderby_columns)?>">País</a></th>
                <th><a href="<?= generateQueryString('RESULTADO_TIPO', $orderby, $order, $orderby_columns)?>">Tipo de Resultado</a></th>
                <th><a href="<?= generateQueryString('RESULTADO_VALOR', $orderby, $order, $orderby_columns)?>">Valor del Resultado</a></th>
            </tr>
        </thead>
        <tbody>
